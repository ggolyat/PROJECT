...
void process_loop(int ii, int loop) 
{
    try 
    {
        ...
        string keys_file_path = "testdata2/random_data_2_" + to_string(temp) + "_" + to_string(loop) + ".bin";
...
int main(int argc, char* argv[]) 
{
    ...
    std::string filename = "result/2_1_" + to_string(int((1.0-sp[ii]*2)*100 + 0.001)) + "_insert_time.csv";
    exportToCSV(insert_cost_time, filename);
    plt::scatter(ploty1, ploty2);
    plt::save("result/2_1_" + to_string(int((1.0-sp[ii]*2)*100 + 0.001)) +"_mix.jpg");
    // plt::scatter(plotx, ploty1);
    // plt::save("result/1_2_" + to_string(int((1.0-sp[ii]*2)*100 + 0.001)) +"_location.jpg");
    // plt::scatter(plotx, ploty2);
    // plt::save("result/1_2_" + to_string(int((1.0-sp[ii]*2)*100 + 0.001)) +"_time.jpg");
    return 0;
}
